...
import { UserButton, useUser } from '@clerk/clerk-react' // 👈 เปลี่ยนเป็น @clerk/clerk-react

const Navbar = () => {
    const { isSignedIn, user } = useUser()
    const location = useLocation()
    
    // ดึงชื่อผู้ใช้จาก Clerk (แก้คำผิด fullname -> fullName, fisrtName -> firstName)
    const userName = user?.fullName || user?.firstName || user?.primaryEmailAddress?.emailAddress?.split("@")[0] || "User"

  return (
    <header className="w-full max-w-305 mx-auto bg-white/90 backdrop-blur xl:rounded-b-xl sticky top-0 z-40 px-6 py-4 flex items-center justify-between border-slate-200">
      ...
      {isSignedIn && (
        <div className='flex items-center gap-4'>
            <Link to="/sessions" className='md:hidden text-xs font-medium text-slate-600 hover:text-primary flex-center gap-1'>
